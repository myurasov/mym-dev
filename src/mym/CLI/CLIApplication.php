...
class CLIApplication
{
  ...
  public function getTimePassed($return_as_string = false)
  {
    ...
    return $return_as_string
      ? $this->_format_time($this->time_total)
      : $this->time_total;
  }
  ...
  */
  ...
  public function displayHelp()
  {
    ...
    {
      $text = Text::textAlign($this->options['script_description'], Text::TEXT_ALIGN_LEFT,
        $this->options['max_output_width'], "\n", true, 1, 0);
      echo "\n\n$text";
    }
      ...
      foreach ($this->declared_parameters as $declared_parameter_name => $declared_parameter)
      {
        switch ($this->_get_declared_parameter_type($declared_parameter_name))
        {
          ...
          case self::PARAM_TYPE_TIME_SEC:
          {
            ...
            $default_value = $declared_parameter['default'] .
              ' (' . Strings::formatTime($this->_time_sec_to_int($declared_parameter['default'])) . ')';
            break;
          }
        ...
        $text = sprintf("* %s [%s]; default: %s",
          $declared_parameter_name .
            ($declared_parameter['alias'] == $declared_parameter_name
              ? '' : " ({$declared_parameter['alias']})"),
          $type_name, $default_value);

        $text = Text::textAlign($text, Text::TEXT_ALIGN_LEFT,
          $this->options['max_output_width'], "\n", true, 1, 0);
    ...
    if (!is_null($this->progress_refresh_interval))
    {
      ...
      if ($time_for_progress  || $last_item_reached)
      {
        ...
        $this->progress_tags['%eta%'] = $progress_info_eta == -1 ? '?'
          : Strings::formatTime($progress_info_eta, $this->options['progress_time_precision'], true, 1, true);

        // %time_passed% - time passed
        $this->progress_tags['%time_passed%'] = $progress_info_time_passed == -1 ? '?'
          : Strings::formatTime($progress_info_time_passed, $this->options['progress_time_precision'], true, 1, true);
  ...
  private function _log_end()
  {
    if ($this->_message_log_enabled() && $this->message_log_fp)
    {
      // Write footer
      fwrite($this->message_log_fp,
        sprintf("[Log finished at %s (+ %s)]\n",
          date('r'), $this->_format_time(microtime(true) -
            $this->message_log_start_time)));
  ...
  private function _read_parameter_value($arg_value, $declared_parameter_name)
  {
    ...
    else if ($type == self::PARAM_TYPE_BOOLEAN)
    {
      ...
      {
        return Strings::str2Bool($arg_value);
      }
    }
    else if ($type == self::PARAM_TYPE_ARRAY)
    {
      return Strings::explodeString($arg_value, '+');
    }
  ...
  private function _get_declared_parameter_type($declared_parameter_name)
  {
    ...
    if ($declared_parameter['type'] == self::PARAM_TYPE_AUTO)
    {
      ...
      else
      {
        ...
        else if (is_float($declared_parameter['default']))
        {
          return self::PARAM_TYPE_FLOAT;
        }
  ...
  {
    return Strings::formatTime(
      $time, $this->options['status_time_precision'], true, 1, true);
  }
}